...
def linear(x, w):
    ...
    if isinstance(x[0], list) or isinstance(x[0], Value): # It's a vector since Value is scalar
       pass
...
def main():
    ...
    for step in range(num_steps):
        ...
        if step % 100 == 0:
            ...
            print(f"step {step+1:4d} / {num_steps:4d} | train_loss {loss.data:.4f} | val_loss {val_loss:.4f}")
        else:
            print(f"step {step+1:4d} / {num_steps:4d} | train_loss {loss.data:.4f}", end="\r")
