...
def main():
    steps = 2500
    baseline_log = "baseline_scale_log.txt"
    pga_log = "pga_scale_log.txt"
    
    cmd_baseline = [sys.executable, "microgpt_scale.py", "--steps", str(steps), "--log", baseline_log, "--seed", "42"]
    cmd_pga = [sys.executable, "pga_scale.py", "--steps", str(steps), "--log", pga_log, "--seed", "42"]
    
    print(f"Starting Scale Up Benchmark (v6-pga) for {steps} steps...")
    print(f"Config: n_embd=64, n_layer=4, block_size=64")
    ...
    subprocess.run([sys.executable, "plot_v6.py", "--baseline", baseline_log, "--pga", pga_log])
