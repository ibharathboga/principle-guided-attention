...
def run_layers(layers, x, P, rmsnorm, idx_device):
    ...
    for layer in layers:
        ...
        if P is not None:
            # FIX 1: Apply P only to Q and K (Not V)
...
def compute_P_from_store_adaptive(token_store, token_ids, x_queries, retrieve_k=RETRIEVE_K):
    ...
    try:
        ...
        return P, {'avg_rank': D, 'svd_fails': B * T}

    # FIX 4: Adaptive Energy Threshold
    total_energy = S.sum(dim=-1, keepdim=True)
    cum_energy = S.cumsum(dim=-1)
    
    # Mask capturing 90% of the energy
    keep_mask = cum_energy < (ENERGY_THRESHOLD * total_energy)
    
    # Ensure at least 1 principle component is kept
    keep_mask[:, 0] = True
    
    # Zero out unkept singular vectors organically for fully batched operations
    ...
    return P, {'avg_rank': avg_rank, 'svd_fails': 0}
...
# ═══════════════════════════════════════════════════
#  2. E12 Propagative PGA with Buffer
# ═══════════════════════════════════════════════════

class PropPGABuffer(nn.Module):
    def __init__(self, vocab_size):
        super().__init__()
        self.vocab_size = vocab_size
        self.wte = nn.Embedding(vocab_size, N_EMBD)
        ...
        # FIX 2: Deep MLP Projection to match feature depths 
    ...
    def forward(self, idx, targets=None):
        ...
        # Undocumented Compromise: The query projection is detached from the main computation graph. 
        # This means the embedding layer won't receive gradients from the retrieval 
        # mechanism. This is a deliberate compromise to avoid instability from backpropping through SVD context gathering.
        x_proj = self.query_proj(x.detach())
        
        # FIX 3: Propagative SVD - Computed once here and passed to all layers in `run_layers`
        P, svd_stats = compute_P_from_store_adaptive(self.token_store, idx, x_proj)
        self.last_svd_stats = svd_stats

        # Layers apply FIX 1 (Q/K only) internally
