"""
E13 — Principle Augmentation Pipeline
  1. Baseline MicroGPT
...
def run_experiment():
    log("=" * 60)
    log("E13 — Principle Augmentation (Virtual Tokens)")
    log(f"Config: {N_LAYER}L, embd={N_EMBD}, lr={LR}, steps={STEPS}")
    log(f"E13 PGA: cap={MAX_PER_TOKEN}/token, adaptive_energy={ENERGY_THRESHOLD}, MLP Query, Augmentation (Rank Adaptive)")
    ...
    plt.tight_layout()
    combined_path = os.path.join(RESULTS_DIR, 'e13_combined.png')
    ...
    report = f"""# E13 — Principle Augmentation (Virtual Tokens)
