"""
E8 — Propagative PGA + Observation Buffer Experiment Pipeline
...
def load_data():
    if not os.path.exists(DATA_FILE):
        # Try copying from e7 first
        e7_data = os.path.join(os.path.dirname(__file__), "..", "e7-propagative", "shakespeare.txt")
        if os.path.exists(e7_data):
            shutil.copy2(e7_data, DATA_FILE)
            log(f"Copied Shakespeare from e7")
        else:
...
def run_parallel():
    ...
    log("=" * 60)
    log("E8 — Propagative PGA + Observation Buffer Experiment")
    ...
    # Colors
    C_BASE = '#2196F3'   # Blue
    C_PGA = '#FF5722'    # Red-Orange
    C_BUF = '#4CAF50'    # Green

    # Combined plot (all on one graph)
    plt.figure(figsize=(14, 8))

    # Training losses (dotted)
    ...
    plt.title(
        f'E8 — Baseline vs Prop-PGA vs Prop-PGA+Buffer\n'
        f'{N_LAYER} Layers | lr={LR} | Shakespeare | {STEPS} Steps | Buffer={BUFFER_CAPACITY}',
        fontsize=13
    )
    ...
    combined_path = os.path.join(RESULTS_DIR, 'e8_combined.png')
    ...
    plt.suptitle(
        f'E8 — 3-Way Comparison ({N_LAYER} Layers, lr={LR}, Shakespeare)',
        fontsize=14, fontweight='bold'
    )
    plt.tight_layout()

    split_path = os.path.join(RESULTS_DIR, 'e8_results.png')
    ...
    report = f"""# E8 — Propagative PGA + Observation Buffer Report
...
- SVD computed from sliding window of {WINDOW_SIZE} adjacent embeddings
- No cross-sequence memory

### Prop-PGA+Buffer — NEW in e8
- **Observation Buffer**: FIFO ring buffer of {BUFFER_CAPACITY} vectors
...
- **Propagative**: Same P across all {N_LAYER} layers (compute once from embeddings)
