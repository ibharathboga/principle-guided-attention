"""
E14 — Stabilized Principle Augmentation Pipeline
  1. Baseline MicroGPT
  2. Prop PGA Augmentation (SVD Virtual Tokens)
...
def log(msg, model_name="SYSTEM"):
    ...
    out = f"[{ts}] [{model_name}] {msg}"
    print(out, flush=True)
    with open(os.path.join(RESULTS_DIR, "output.txt"), "a", encoding="utf-8") as f:
        f.write(out + "\n")
...
def train_model(model, model_name, train_data, val_data, store_save_path=None):
    ...
    if hasattr(model, 'token_store'):
        ...
        if len(svd_ranks) > 0:
            result['avg_rank'] = sum(svd_ranks) / len(svd_ranks)
...
def run_experiment():
    log("=" * 60)
    log("E14 — Stabilized Principle Augmentation (Batch=16, Cosine LR)")
    log(f"Config: {N_LAYER}L, embd={N_EMBD}, lr={LR}, steps={STEPS}, batch={BATCH_SIZE}")
    log(f"E14 PGA: cap={MAX_PER_TOKEN}/token, adaptive_energy={ENERGY_THRESHOLD}, MLP Query, Augmentation (Rank Adaptive)")
    ...
    from models import PropPGAAugment
    ...
    with ProcessPoolExecutor(max_workers=1) as executor:
        ...
        torch.manual_seed(SEED)
        np.random.seed(SEED)
        pga_model = PropPGAAugment(vocab_size)
        store_path = os.path.join(RESULTS_DIR, "token_store.pt")
        pga_result = train_model(pga_model, "PropPGAAugment", train_data, val_data, store_save_path=store_path)

        results = {"PropPGAAugment": pga_result}
    ...
    colors = {
        'Baseline': '#2196F3',
        'PropPGAAugment': '#FF9800', # Orange for Augment
    }
    markers = {
        'Baseline': 'o',
        'PropPGAAugment': 's',
    }
    order = ['Baseline', 'PropPGAAugment']
    ...
    plt.title(
        f'E13 — Principle Augmentation (Virtual Tokens)\n'
    ...
    plt.tight_layout()
    combined_path = os.path.join(RESULTS_DIR, 'e14_combined.png')
    ...
    # E14 specific note
    vtoken_note = f"Principles used: {RETRIEVE_K} max retrieved, flattened to ~{results['PropPGAAugment'].get('avg_rank', 0):.1f} virtual tokens."

    report = f"""# E14 — Stabilized Principle Augmentation
...
|---|---|---|
| Train Loss | {results['Baseline']['final_train']:.4f} | {results['PropPGAAugment']['final_train']:.4f} |
| Val Loss | {results['Baseline']['final_val']:.4f} | {results['PropPGAAugment']['final_val']:.4f} |
| Gap | {gaps['Baseline']:.4f} | {gaps['PropPGAAugment']:.4f} |
| Step Time | {results['Baseline']['avg_step_ms']:.1f}ms | {results['PropPGAAugment']['avg_step_ms']:.1f}ms |
